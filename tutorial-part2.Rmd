---
title: "R Programing - Best Practices Part 2"
...
```{r}
numeric_vars <- c(1:4)
means <- lapply(iris[numeric_vars], mean)
head(means)
```
...
means <- lapply(iris[numeric_vars], get_mean)
means
```
...
squares <- data.frame(lapply(iris[numeric_vars], (function(x) x+1)))
head(squares)
```
